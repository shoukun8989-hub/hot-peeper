...
export async function call(sync, action, payload = {}) {
  ...
  if (!json.ok) throw new Error(explain(json.error));
  return json;
}

/** スプレッドシート側から返ってきた文言を、やることが分かる形に言い換える */
export function explain(error) {
  const t = String(error || '同期先でエラーが起きました。');

  if (/UrlFetchApp|script\.external_request/.test(t)) {
    return 'スプレッドシート側で、外部への通信がまだ許可されていません。'
         + 'Apps Script を開いて、関数一覧から authorize を選んで1回実行し、出てくる画面で「許可」してください。'
         + 'そのあと「デプロイを管理 → 編集 → バージョン: 新バージョン」で更新すると使えるようになります。';
  }
  if (/不明な操作です/.test(t)) {
    return 'スプレッドシート側のコードが古いようです。Code.gs を貼り直してから、'
         + '「デプロイを管理 → 編集 → バージョン: 新バージョン」で更新してください。（' + t + '）';
  }
  if (/合言葉/.test(t)) {
    return t + ' Apps Script の SECRET と、この画面の合言葉を同じ文字列にしてください。';
  }
  return t;
}
